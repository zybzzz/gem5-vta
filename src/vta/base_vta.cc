...
namespace gem5{
    BaseVTA::BaseVTA(const Params &params):
        ...
        hardware_version(params.HW_VER), input_data_width(1 << params.LOG_INP_WIDTH),
        weight_data_width(1 << params.LOG_WGT_WIDTH), accumulator_data_width(1 << params.LOG_ACC_WIDTH),
        batch_size(1 << params.LOG_BATCH), block_size(1 << params.LOG_BLOCK),
        uop_buffer_size(1 << params.LOG_UOP_BUFF_SIZE), input_buffer_size(1 << params.LOG_INP_BUFF_SIZE),
        weight_buffer_size(1 << params.LOG_WGT_BUFF_SIZE), accumulator_buffer_size(1 << params.LOG_ACC_BUFF_SIZE){
        DPRINTF(BaseVTAFlag, "===========================================\n");

        DPRINTF(BaseVTAFlag, "initializing gem5-vta ... user configuration parameters are as follows: \n");
        ...
        DPRINTF(BaseVTAFlag, "accumulator_data_width: %10d\n", accumulator_data_width);
